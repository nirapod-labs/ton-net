...
- A dictionary descent holds its edge labels inline rather than in a vector per
  level. A lookup now costs one allocation, the run of key bits it spreads the
  caller's key into, whatever the depth; before, it cost one for each label on
  the way down that was not empty, which on a sparse tree is one per level. A walk costs the key
  vector it hands back per entry, where it copied the key prefix onto the heap
  once per node besides. No signature changes and the trees are the ones the
  mainnet root hashes already held them to.
