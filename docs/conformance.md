...
fault (`core/tests/dictionary.rs`). The only thing that catches that
...
- **A second party**, for the values a single server could otherwise invent. A block
  ...
  being pinned (`core/tests/verify.rs`, `decode.rs`, `validators.rs`).
...
reported (`core/tests/verify.rs`). The shard block that carries a
...
for any other root (`core/tests/proof.rs`).
...
(`core/tests/cell/augmented.rs`). Every fork is checked to summarise the
...
under, `e55bf967…` and `80cbafe6…` (`core/tests/cell/whole_block.rs`).
...
`core/tests/cell/mainnet_boc.rs`). This is the byte-for-byte agreement
...
computed for the trees they cover (`core/tests/cell/hashing.rs`).
...
server's proof cell byte for byte (`core/tests/cell/merkle.rs`).
...
The rule is established two ways (`core/tests/signature.rs`). A
...
(`core/tests/validators.rs`). Two rounds twelve million blocks apart are
...
reusing it (`core/tests/chain.rs`). A one-link answer in the newer Simplex
...
(`core/tests/wire.rs`). The block-proof types are anchored harder: two whole
...
liteserver accepted in the feasibility spike (`core/tests/vectors.rs`).
...
(`core/tests/epoch.rs`). A change to what the engine accepts fails the test, which
...
session and read a plausible head from a real node (`core/tests/mainnet_adnl.rs`,
`core/tests/mainnet_lite.rs`, `core/tests/mainnet_client.rs`), and that the
bundled mainnet snapshot has not decayed past a floor of reachable liteservers or let its
pinned block recede out of reach (`core/tests/snapshot.rs`). The captured fixtures
...
| Area | Tests | Fixtures |
|---|---|---|
| Bag of cells, real bytes | `core/tests/cell/mainnet_boc.rs` | `fixtures/account-proof.hex` |
| Whole block hashes to its id | `core/tests/cell/whole_block.rs` | `fixtures/block-masterchain.hex`, `fixtures/block-basechain.hex` |
| Augmented dictionaries | `core/tests/cell/augmented.rs` | `fixtures/block-basechain.hex` |
| Cell hashing, Merkle proofs | `core/tests/cell/hashing.rs`, `merkle.rs` | `fixtures/account-proof.hex`, `fixtures/block-basechain.hex` |
| Hostile and corrupted input | `core/tests/cell/hostile.rs` | `fixtures/account-proof.hex` |
| Account read verification | `core/tests/verify.rs`, `proof.rs` | `fixtures/read-masterchain.txt`, `read-basechain.txt`, `read-absent.txt` |
| Dictionary engine | `core/tests/dictionary.rs` | `fixtures/chain.hex`, `read-*.txt` |
| Account decode | `core/tests/decode.rs` | `fixtures/active-account.hex`, `uninit-account.hex`, `storage-extra-account.hex`, `account-proof.hex` |
| Proof chain and signatures | `core/tests/chain.rs`, `validators.rs`, `signature.rs` | `fixtures/chain.hex`, `one-link-simplex.hex` |
| TL wire format | `core/tests/wire.rs` | `fixtures/one-link-ordinary.hex`, `one-link-simplex.hex` |
| ADNL handshake vectors | `core/tests/vectors.rs` | inline |
